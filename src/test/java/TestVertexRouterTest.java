...
public class TestVertexRouterTest {
	private Vertx vertx;

	@Before
	public void setUp(TestContext context) {
		vertx = Vertx.vertx();
		TestVertexRouter vertex = new TestVertexRouter();
		vertex.setPathMainDir(MyTestSupport.getTestDir());
		//vertx.deployVerticle(TestVertexRouter.class.getName(), context.asyncAssertSuccess());
	...
	public void access(TestContext context) {
		...
		vertx.createHttpClient().getNow(8080, "localhost", "/api/fs/", response -> {
	...
	public void fatherDirFalse(TestContext context){
		...
		vertx.createHttpClient()
			.getNow(8080, "localhost", "/api/fs/1/5", response -> {
			response.bodyHandler(body -> {
				try {
					JsonObject json = new JsonObject(body.toString());
					context.assertTrue(json.getBoolean("status"));
	...
	public void fatherDirTrue(TestContext context){
		final Async async = context.async();
		vertx.createHttpClient()
				.getNow(8080, "localhost", "/api/fs", response -> {
					response.bodyHandler(body -> {
						try {
							JsonObject json = new JsonObject(body.toString());
							context.assertTrue(json.getBoolean("status"));
	...
	public void getFile(TestContext context){
		final Async async = context.async();
		vertx.createHttpClient().getNow(8080, "localhost", "/api/fs/1.txt", response -> {
			response.bodyHandler(body -> {
				try {
					JsonObject json = new JsonObject(body.toString());
					context.assertTrue(json.getBoolean("status"));
	...
	public void badRequest(TestContext context){
		final Async async = context.async();
		vertx.createHttpClient().getNow(8080, "localhost", "/api/fs/2.txt", response -> {
			response.bodyHandler(body -> {
				try {
					JsonObject json = new JsonObject(body.toString());
